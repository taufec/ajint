...
ssh-keygen -q -t ed25519 -N '' -C "ajint:$CONTROL_REPO" -f "$KEY"
mkdir -p "$SSH_DIR"
chmod 700 "$SSH_DIR"
touch "$AUTH_KEYS"
chmod 600 "$AUTH_KEYS"
...
Private machine control plane created by Ajint.
...
GIT_COMMITTER_EMAIL='bridge@localhost' \
  git -C "$WORK/repo" commit -m 'Initialize Ajint control plane' >/dev/null
